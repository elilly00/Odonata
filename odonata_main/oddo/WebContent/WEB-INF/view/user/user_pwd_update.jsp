                ...
                <div class="card mask-custom">
                  <div class="card-body p-5">
                    <div class="my-4">
                      ...
                      <form action="<%= request.getContextPath() %>/updatePwd.us" method="post" id="updatePwdForm" name="updatePwdForm" onsubmit="return insertValidate();">
    ...
    <script>
        ...
        $('#newPwd1').blur(function() {
            ...
            if(!regExpPwd.test(userPwd) && !(userPwd.length < 8 || userPwd.length > 12)) {
                $("label[for='newPwd1']").text('비밀번호: 사용 가능').css({'color':'green', 'font-weight':'bold'});
                isPwdUsable = true;
            } else {
                $("label[for='newPwd1']").text('비밀번호: 사용 불가').css({'color':'red', 'font-weight':'bold'});
                isPwdUsable = false;
            }
        });
        ...
        var pwd2 = $('#newPwd2').val();
            if(pwd1 == pwd2) {
                $("label[for='newPwd2']").text('비밀번호 확인: 일치').css({'color':'green', 'font-weight':'bold'});
                isPwdChecked = true;
            } else {
                $("label[for='newPwd2']").text('비밀번호 확인: 불일치').css({'color':'red', 'font-weight':'bold'});
                isPwdChecked = false;
            }
        });
