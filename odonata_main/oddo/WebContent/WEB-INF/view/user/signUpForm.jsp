              ...
              <div class="card mask-custom">
                <div class="card-body p-5">
                  <div class="my-4">
                    ...
                    <form action="<%= request.getContextPath() %>/insert.us" method="post" id="joinForm" name="joinForm"
                      ...
                      <div class="row">
                        ...
                        <div class="col-12 col-md-6 mb-4">
                          <div class="form-floating form-white">
                            <input type="password" id="uPwd1" name="uPwd1" class="form-control form-control-lg" />
                            <label class="form-label" for="uPwd1">비밀번호</label>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="row">
                        ...
                        <div class="col-12 col-md-6 mb-4">
                          <div class="form-floating form-white">
                            <input type="password" id="uPwd2" name="uPwd2" class="form-control form-control-lg" />
                            <label class="form-label" for="uPwd2">비밀번호 확인</label>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="row">
                        <div class="col-12 col-md-6 mb-4">
                          <div class="form-floating form-white">
                            <input type="text" id="uEmailId" name="uEmailId" class="form-control form-control-lg" />
                            <label class="form-label" for="uEmailId">이메일</label>
                          </div>
                        </div>
                        <div class="col-12 col-md-6 mb-4">
                          <div class="form-floating form-white">
                      ...
                      <div class="form-floating form-white mb-4">
                        <input type="tel" id="uPhone" name="uPhone" class="form-control form-control-lg" />
                        <label class="form-label" for="uPhone">휴대전화</label>
                      </div>

                      <div class="form-floating form-white mb-4">
                        <input type="text" id="uBirth" name="uBirth" class="form-control form-control-lg" />
                        <label class="form-label" for="uBirth">생년월일</label>
                      </div>

                      <div class="form-check d-flex justify-content-center mb-4">
                        <input class="form-check-input me-2" type="checkbox" value="" id="regTerms" checked />
  ...
  <script>
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (data.trim() == '0' && (!regExpId.test(userId.val()) && regExpIdStart.test(userId.val()))) {
              $("label[for='uId']").text('아이디: 사용 가능').css({
                'color': 'green',
                'font-weight': 'bold'
              });
              isIdUsable = true;
              ...
              $("label[for='uId']").text('아이디: 사용 불가').css({
                'color': 'red',
                'font-weight': 'bold'
              });
              isIdUsable = false;
    ...
    $('#uPwd1').blur(function () {
      ...
      if (!regExpPwd.test(userPwd) && !(userPwd.length < 8 || userPwd.length > 12)) {
        $("label[for='uPwd1']").text('비밀번호: 사용 가능').css({
          'color': 'green',
          'font-weight': 'bold'
        });
        isPwdUsable = true;
      } else {
        $("label[for='uPwd1']").text('비밀번호: 사용 불가').css({
          'color': 'red',
          'font-weight': 'bold'
        });
        isPwdUsable = false;
    ...
    $('#uPwd2').keyup(function () {
      ...
      var pwd2 = $('#uPwd2').val();
      if (pwd1 == pwd2) {
        $("label[for='uPwd2']").text('비밀번호 확인: 일치').css({
          'color': 'green',
          'font-weight': 'bold'
        });
        isPwdChecked = true;
      } else {
        $("label[for='uPwd2']").text('비밀번호 확인: 불일치').css({
          'color': 'red',
          'font-weight': 'bold'
        });
        isPwdChecked = false;
    ...
    $('#uName').blur(function () {
      ...
      if (regExpName.test(name) || name.length < 2) {
        $("label[for='uName']").text('이름: 올바르지 않음').css({
          'color': 'red',
          'font-weight': 'bold'
        });
        nameChk = false;
      } else {
        $("label[for='uName']").text('이름').css({
          'color': 'green',
          'font-weight': 'bold'
        });
        nameChk = true;
      ...
      if (!userEmail) {
        ...
        $.ajax({
          ...
          data: {
            ...
            inputDomain: emailDomain.val()
          },
          success: function (data) {
            console.log(data);
            if (data.trim() == '0') {
              $("label[for='uEmailId']").text('이메일: 사용 가능').css({
                'color': 'green',
                'font-weight': 'bold'
              });
              isEmailUsable = true;
              ...
              $("label[for='uEmailId']").text('이메일: 사용 불가').css({
                'color': 'red',
                'font-weight': 'bold'
              });
              isEmailUsable = false;
    ...
    $('#uPhone').blur(function () {
      ...
      if (regExpPhone.test(phone) && phone.length == 13) {
        $("label[for='uPhone']").text('휴대전화').css({
          'color': 'green',
          'font-weight': 'bold'
        });
        phoneChk = true;
      } else {
        $("label[for='uPhone']").text('휴대전화: 올바르지 않음').css({
          'color': 'red',
          'font-weight': 'bold'
        });
        phoneChk = false;
    ...
    $('#uBirth').blur(function () {
      ...
      if (regExpBirth.test(birth) && birth.length == 10) {
        ...
        $("label[for='uBirth']").text('생년월일: 올바르지 않음').css({
          'color': 'red',
          'font-weight': 'bold'
        });
        birthChk = false;
    ...
    function insertValidate() {
      ...
      if (confirmMail) {
        if (isIdUsable && isIdChecked && isPwdUsable && isPwdChecked && nameChk && isEmailUsable && isEmailChecked &&
          phoneChk && birthChk) {
